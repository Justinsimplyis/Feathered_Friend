...
class ObservationsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        userId = intent.getIntExtra("userId", -1) // Retrieve the logged-in userId from Intent
        ...
        observationsListView.setOnItemClickListener { _, _, position, _ ->
            ...
            intent.putExtra("userId", userId) // Pass userId to details activity
    ...
    private fun showAddObservationDialog() {
        ...
        dialogSaveButton.setOnClickListener {
            ...
            if (title.isNotEmpty() && details.isNotEmpty() && date.isNotEmpty() && location.isNotEmpty()) {
                dbHelper.addObservation(title, details, date, location, userId) // this passes the userId when adding observation
                loadObservations()
